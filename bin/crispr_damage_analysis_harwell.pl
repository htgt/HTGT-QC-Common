...
my ($seq_filename, $scf_filename,  $het_scf_filename, $target_start, $target_end,
    $target_chr,   $target_strand, $species,          $dir,          $ref_seq_filename
);
GetOptions(
    ...
    'species=s'       => \$species,
    'ref-seq-file=s'  => \$ref_seq_filename,
) or pod2usage(2);
...
my $ref_seq_file;
if ( $ref_seq_filename ) {
    $ref_seq_file = file( $ref_seq_filename )->absolute;
}
...
if ( $scf_filename ) {
    ...
    $work_dir = dir( $DEFAULT_QC_DIR )->subdir( $scf_file->basename );
    $work_dir->mkpath;
...
sub variant_scf_to_fasta {
    ...
    $params{ref_seq_file} = $ref_seq_file if $ref_seq_file;
